  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 font-display">Sedes</h1>
        ...
        <Link href="/super-admin/colegios/nuevo" className="btn-primary">
          <i className="ti ti-plus text-sm" aria-hidden="true"/> Nueva sede
        </Link>
      </div>

      <div className="grid grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Sedes',        val: colegios?.length ?? 0,                                          icon: 'ti-building-school', color: 'text-blue-600',   bg: 'bg-blue-50' },
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              {['Sede','RUT','Plan','Alumnos','Usuarios','Creado','Acciones'].map(h => (
          ...
          <tbody>
            {(!colegios || colegios.length === 0) ? (
              <tr><td colSpan={7} className="px-4 py-12 text-center">
                ...
                <p className="text-slate-400 text-sm">No hay sedes registradas todavía.</p>
                <Link href="/super-admin/colegios/nuevo" className="btn-primary mt-3 inline-flex">
                  <i className="ti ti-plus text-sm" aria-hidden="true"/> Crear primera sede
                </Link>
              </td></tr>
