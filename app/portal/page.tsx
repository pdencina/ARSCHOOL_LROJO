...
export default async function PortalPage() {
  ...
  if (rol === 'alumno') {
    const { data: va } = await supabase.from('usuario_alumno').select('alumno_id').eq('usuario_id', user.id).single()
    alumnoId = (va as any)?.alumno_id ?? null
  } else {
    const { data: ta } = await supabase.from('tutor_alumnos').select('alumno_id').eq('tutor_id', user.id).limit(1).single()
    alumnoId = (ta as any)?.alumno_id ?? null
  }

  if (alumnoId) {
    const { data: a } = await supabase.from('alumnos').select('*, familias(nombre_apoderado, apellido_apoderado, email)').eq('id', alumnoId).single()
    alumno = a
  }
  ...
  const [{ data: asistencias }, { data: calificaciones }, { data: comunicados }, { data: cobros }] = await Promise.all([
    alumnoId ? supabase.from('asistencias').select('estado').eq('alumno_id', alumnoId) : Promise.resolve({ data: [] }),
    alumnoId ? supabase.from('calificaciones').select('nota').eq('alumno_id', alumnoId) : Promise.resolve({ data: [] }),
    u.colegio_id ? supabase.from('comunicados').select('id, titulo, tipo, enviado_at').eq('colegio_id', u.colegio_id).order('enviado_at', { ascending: false }).limit(5) : Promise.resolve({ data: [] }),
    alumnoId && rol === 'apoderado' ? supabase.from('cobros').select('estado, monto, monto_pagado').eq('alumno_id', alumnoId).eq('mes', mes).eq('anio', anio) : Promise.resolve({ data: [] }),
  ])
